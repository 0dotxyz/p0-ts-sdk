 ...
 *
 * Rates here use the reserve state as of its last refresh. klend-sdk >= 12 instead projects
 * accrual to a ledger instant (`getEstimatedDebtAndSupply`) and adds a reserve-rewards supply APR
...
export function getKaminoTotalSupply(reserve: KaminoReserve): Decimal {
  const liquidityAvailableAmount = new Decimal(reserve.liquidity.totalAvailableAmount.toString());
