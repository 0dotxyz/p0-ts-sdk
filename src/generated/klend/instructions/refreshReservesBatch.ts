...
export const REFRESH_RESERVES_BATCH_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([
  144, 110, 26, 103, 162, 204, 252, 147,
]);

export function getRefreshReservesBatchDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(REFRESH_RESERVES_BATCH_DISCRIMINATOR);
}
...
export type RefreshReservesBatchInstructionDataArgs = { skipPriceUpdates: boolean };
...
export function getRefreshReservesBatchInstructionDataEncoder(): FixedSizeEncoder<RefreshReservesBatchInstructionDataArgs> {
  return transformEncoder(
    ...
    (value) => ({ ...value, discriminator: REFRESH_RESERVES_BATCH_DISCRIMINATOR })
  );
}
...
export type ParsedRefreshReservesBatchInstruction<
  TProgram extends string = typeof KAMINO_LENDING_PROGRAM_ADDRESS,
> = { programAddress: Address<TProgram>; data: RefreshReservesBatchInstructionData };
...
export function parseRefreshReservesBatchInstruction<TProgram extends string>(
  ...
  return {
    ...
    data: getRefreshReservesBatchInstructionDataDecoder().decode(instruction.data),
  };
}
