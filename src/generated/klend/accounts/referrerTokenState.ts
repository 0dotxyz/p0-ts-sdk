...
export const REFERRER_TOKEN_STATE_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([
  39, 15, 208, 77, 32, 195, 105, 56,
]);

export function getReferrerTokenStateDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(REFERRER_TOKEN_STATE_DISCRIMINATOR);
}
...
export function getReferrerTokenStateEncoder(): FixedSizeEncoder<ReferrerTokenStateArgs> {
  return transformEncoder(
    ...
    (value) => ({ ...value, discriminator: REFERRER_TOKEN_STATE_DISCRIMINATOR })
  );
}
...
export function getReferrerTokenStateCodec(): FixedSizeCodec<
  ...
  ReferrerTokenState
> {
  return combineCodec(getReferrerTokenStateEncoder(), getReferrerTokenStateDecoder());
}
...
): Account<ReferrerTokenState, TAddress> | MaybeAccount<ReferrerTokenState, TAddress> {
...
export async function fetchMaybeReferrerTokenState<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig
...
export async function fetchAllReferrerTokenState(
  ...
  const maybeAccounts = await fetchAllMaybeReferrerTokenState(rpc, addresses, config);
...
export async function fetchAllMaybeReferrerTokenState(
  ...
  return maybeAccounts.map((maybeAccount) => decodeReferrerTokenState(maybeAccount));
}
