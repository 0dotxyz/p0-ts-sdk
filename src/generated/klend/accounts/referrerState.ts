...
export function getReferrerStateDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(REFERRER_STATE_DISCRIMINATOR);
}
...
export function getReferrerStateCodec(): FixedSizeCodec<ReferrerStateArgs, ReferrerState> {
...
export function decodeReferrerState<TAddress extends string = string>(
  ...
  return decodeAccount(encodedAccount as MaybeEncodedAccount<TAddress>, getReferrerStateDecoder());
}
...
export async function fetchAllReferrerState(
  ...
  const maybeAccounts = await fetchAllMaybeReferrerState(rpc, addresses, config);
