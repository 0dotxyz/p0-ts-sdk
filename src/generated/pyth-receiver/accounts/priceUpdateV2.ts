...
export const PRICE_UPDATE_V2_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([
  34, 241, 35, 99, 157, 126, 244, 205,
]);

export function getPriceUpdateV2DiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(PRICE_UPDATE_V2_DISCRIMINATOR);
}
...
export function decodePriceUpdateV2<TAddress extends string = string>(
  ...
  return decodeAccount(encodedAccount as MaybeEncodedAccount<TAddress>, getPriceUpdateV2Decoder());
}
...
export async function fetchAllPriceUpdateV2(
  ...
  const maybeAccounts = await fetchAllMaybePriceUpdateV2(rpc, addresses, config);
