...
export function getOraclePricesDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(ORACLE_PRICES_DISCRIMINATOR);
}
...
export type OraclePricesArgs = { oracleMappings: Address; prices: Array<DatedPriceArgs> };
...
export function getOraclePricesCodec(): FixedSizeCodec<OraclePricesArgs, OraclePrices> {
...
export function decodeOraclePrices<TAddress extends string = string>(
  ...
  return decodeAccount(encodedAccount as MaybeEncodedAccount<TAddress>, getOraclePricesDecoder());
}
