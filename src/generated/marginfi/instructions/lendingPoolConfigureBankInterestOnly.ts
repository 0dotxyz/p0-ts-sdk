/**
 * This code was AUTOGENERATED using the Codama library.
 * Please DO NOT EDIT THIS FILE, instead use visitors
 * to add features, then rerun Codama to update it.
 *
 * @see https://github.com/codama-idl/codama
 */

import {
  combineCodec,
  fixDecoderSize,
  fixEncoderSize,
  getBytesDecoder,
  getBytesEncoder,
  getStructDecoder,
  getStructEncoder,
  SOLANA_ERROR__PROGRAM_CLIENTS__INSUFFICIENT_ACCOUNT_METAS,
  SolanaError,
  transformEncoder,
  type AccountMeta,
  type AccountSignerMeta,
  type Address,
  type Codec,
  type Decoder,
  type Encoder,
  type Instruction,
  type InstructionWithAccounts,
  type InstructionWithData,
  type ReadonlyAccount,
  type ReadonlySignerAccount,
  type ReadonlyUint8Array,
  type WritableAccount,
} from "@solana/kit";
import {
  getAccountMetaFactory,
  type InstructionAccountInput,
  type InstructionAccountInputAddress,
  type InstructionSignerInput,
  type ResolvedInstructionAccount,
  type ResolvedInstructionAccountMeta,
} from "@solana/kit/program-client-core";
import { MARGINFI_PROGRAM_ADDRESS } from "../programs";
import {
  getInterestRateConfigOptDecoder,
  getInterestRateConfigOptEncoder,
  type InterestRateConfigOpt,
  type InterestRateConfigOptArgs,
} from "../types";

export const LENDING_POOL_CONFIGURE_BANK_INTEREST_ONLY_DISCRIMINATOR: ReadonlyUint8Array =
  new Uint8Array([245, 107, 83, 38, 103, 219, 163, 241]);

export function getLendingPoolConfigureBankInterestOnlyDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(
    LENDING_POOL_CONFIGURE_BANK_INTEREST_ONLY_DISCRIMINATOR
  );
}

export type LendingPoolConfigureBankInterestOnlyInstruction<
  TProgram extends string = typeof MARGINFI_PROGRAM_ADDRESS,
  TAccountGroup extends string | AccountMeta<string> = string,
  TAccountDelegateCurveAdmin extends string | AccountMeta<string> = string,
  TAccountBank extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountGroup extends string ? ReadonlyAccount<TAccountGroup> : TAccountGroup,
      TAccountDelegateCurveAdmin extends string
        ? ReadonlySignerAccount<TAccountDelegateCurveAdmin> &
            AccountSignerMeta<TAccountDelegateCurveAdmin>
        : TAccountDelegateCurveAdmin,
      TAccountBank extends string ? WritableAccount<TAccountBank> : TAccountBank,
      ...TRemainingAccounts,
    ]
  >;

export type LendingPoolConfigureBankInterestOnlyInstructionData = {
  discriminator: ReadonlyUint8Array;
  interestRateConfig: InterestRateConfigOpt;
};

export type LendingPoolConfigureBankInterestOnlyInstructionDataArgs = {
  interestRateConfig: InterestRateConfigOptArgs;
};

export function getLendingPoolConfigureBankInterestOnlyInstructionDataEncoder(): Encoder<LendingPoolConfigureBankInterestOnlyInstructionDataArgs> {
  return transformEncoder(
    getStructEncoder([
      ["discriminator", fixEncoderSize(getBytesEncoder(), 8)],
      ["interestRateConfig", getInterestRateConfigOptEncoder()],
    ]),
    (value) => ({
      ...value,
      discriminator: LENDING_POOL_CONFIGURE_BANK_INTEREST_ONLY_DISCRIMINATOR,
    })
  );
}

export function getLendingPoolConfigureBankInterestOnlyInstructionDataDecoder(): Decoder<LendingPoolConfigureBankInterestOnlyInstructionData> {
  return getStructDecoder([
    ["discriminator", fixDecoderSize(getBytesDecoder(), 8)],
    ["interestRateConfig", getInterestRateConfigOptDecoder()],
  ]);
}

export function getLendingPoolConfigureBankInterestOnlyInstructionDataCodec(): Codec<
  LendingPoolConfigureBankInterestOnlyInstructionDataArgs,
  LendingPoolConfigureBankInterestOnlyInstructionData
> {
  return combineCodec(
    getLendingPoolConfigureBankInterestOnlyInstructionDataEncoder(),
    getLendingPoolConfigureBankInterestOnlyInstructionDataDecoder()
  );
}

export type LendingPoolConfigureBankInterestOnlyInput<
  TAccountGroup extends InstructionAccountInput = InstructionAccountInput,
  TAccountDelegateCurveAdmin extends InstructionSignerInput = InstructionSignerInput,
  TAccountBank extends InstructionAccountInput = InstructionAccountInput,
> = {
  group: TAccountGroup;
  delegateCurveAdmin: TAccountDelegateCurveAdmin;
  bank: TAccountBank;
  interestRateConfig: LendingPoolConfigureBankInterestOnlyInstructionDataArgs["interestRateConfig"];
};

export function getLendingPoolConfigureBankInterestOnlyInstruction<
  TAccountGroup extends InstructionAccountInput,
  TAccountDelegateCurveAdmin extends InstructionSignerInput,
  TAccountBank extends InstructionAccountInput,
  TProgramAddress extends Address = typeof MARGINFI_PROGRAM_ADDRESS,
>(
  input: LendingPoolConfigureBankInterestOnlyInput<
    TAccountGroup,
    TAccountDelegateCurveAdmin,
    TAccountBank
  >,
  config?: { programAddress?: TProgramAddress }
): LendingPoolConfigureBankInterestOnlyInstruction<
  TProgramAddress,
  ResolvedInstructionAccountMeta<TAccountGroup, InstructionAccountInputAddress<TAccountGroup>>,
  ResolvedInstructionAccountMeta<
    TAccountDelegateCurveAdmin,
    InstructionAccountInputAddress<TAccountDelegateCurveAdmin>
  >,
  ResolvedInstructionAccountMeta<TAccountBank, InstructionAccountInputAddress<TAccountBank>>
> {
  // Program address.
  const programAddress = config?.programAddress ?? MARGINFI_PROGRAM_ADDRESS;

  // Account meta helper.
  const getAccountMeta = getAccountMetaFactory(programAddress, "programId");

  // Original accounts.
  const originalAccounts = {
    group: { value: input.group ?? null, isSigner: false, isWritable: false },
    delegateCurveAdmin: {
      value: input.delegateCurveAdmin ?? null,
      isSigner: true,
      isWritable: false,
    },
    bank: { value: input.bank ?? null, isSigner: false, isWritable: true },
  };
  const accounts = originalAccounts as Record<
    keyof typeof originalAccounts,
    ResolvedInstructionAccount
  >;

  // Original args.
  const args = { ...input };

  return Object.freeze({
    accounts: [
      getAccountMeta("group", accounts.group),
      getAccountMeta("delegateCurveAdmin", accounts.delegateCurveAdmin),
      getAccountMeta("bank", accounts.bank),
    ],
    data: getLendingPoolConfigureBankInterestOnlyInstructionDataEncoder().encode(
      args as LendingPoolConfigureBankInterestOnlyInstructionDataArgs
    ),
    programAddress,
  } as LendingPoolConfigureBankInterestOnlyInstruction<
    TProgramAddress,
    ResolvedInstructionAccountMeta<TAccountGroup, InstructionAccountInputAddress<TAccountGroup>>,
    ResolvedInstructionAccountMeta<
      TAccountDelegateCurveAdmin,
      InstructionAccountInputAddress<TAccountDelegateCurveAdmin>
    >,
    ResolvedInstructionAccountMeta<TAccountBank, InstructionAccountInputAddress<TAccountBank>>
  >);
}

export type ParsedLendingPoolConfigureBankInterestOnlyInstruction<
  TProgram extends string = typeof MARGINFI_PROGRAM_ADDRESS,
  TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[],
> = {
  programAddress: Address<TProgram>;
  accounts: {
    group: TAccountMetas[0];
    delegateCurveAdmin: TAccountMetas[1];
    bank: TAccountMetas[2];
  };
  data: LendingPoolConfigureBankInterestOnlyInstructionData;
};

export function parseLendingPoolConfigureBankInterestOnlyInstruction<
  TProgram extends string,
  TAccountMetas extends readonly AccountMeta[],
>(
  instruction: Instruction<TProgram> &
    InstructionWithAccounts<TAccountMetas> &
    InstructionWithData<ReadonlyUint8Array>
): ParsedLendingPoolConfigureBankInterestOnlyInstruction<TProgram, TAccountMetas> {
  if (instruction.accounts.length < 3) {
    throw new SolanaError(SOLANA_ERROR__PROGRAM_CLIENTS__INSUFFICIENT_ACCOUNT_METAS, {
      actualAccountMetas: instruction.accounts.length,
      expectedAccountMetas: 3,
    });
  }
  let accountIndex = 0;
  const getNextAccount = () => {
    const accountMeta = (instruction.accounts as TAccountMetas)[accountIndex]!;
    accountIndex += 1;
    return accountMeta;
  };
  return {
    programAddress: instruction.programAddress,
    accounts: {
      group: getNextAccount(),
      delegateCurveAdmin: getNextAccount(),
      bank: getNextAccount(),
    },
    data: getLendingPoolConfigureBankInterestOnlyInstructionDataDecoder().decode(instruction.data),
  };
}
